...
class TestNotificationFilters(unittest.TestCase):
    def setUp(self):
        ...
        self.date_filter_mock = patch('django_filters.DateFromToRangeFilter.filter').start()
    ...
    def test_recipient_email_filter(self):
        ...
        # Execute
        result = self.filter.filters['recipient__email'].filter(self.queryset, 'test@example.com')
    ...
    def test_created_at_filter(self):
        ...
        # Execute
        result = self.filter.filters['created_at'].filter(self.queryset, ('2023-01-01', '2023-12-31'))
        
        # Assert
        self.date_filter_mock.assert_called_once_with(self.queryset, ('2023-01-01', '2023-12-31'))
        self.assertEqual(result, self.queryset)
    ...
    def test_filter_fields(self):
        ...
        expected_fields = [
            'id', 'is_read', 'notification_type', 
            'recipient', 'recipient__email', 'created_at'
        ]
