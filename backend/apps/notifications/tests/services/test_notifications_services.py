...
class TestNotificationServices(TestCase):
    def setUp(self):
        ...
        self.notification_get_mock = patch(
            "apps.notifications.models.Notification.objects.get"
        ).start()
    ...
    def test_get_user_preferences_not_found(self):
        ...
        type(user).notification_preferences = PropertyMock(
            side_effect=UserNotificationPreference.DoesNotExist()
        )
    ...
    def test_should_send_notification_with_preferences(self):
        ...
        with patch(
            "apps.notifications.services.get_user_preferences", return_value=preference
        ):
            # Execute
    ...
    def test_should_send_notification_no_preferences(self):
        ...
        with patch(
            "apps.notifications.services.get_user_preferences", return_value=None
        ):
            # Execute
            result = should_send_notification(user, "post", "in_app")
    ...
    def test_notification_create(self):
        ...
        # Setup
        with patch(
            "apps.notifications.services.should_send_notification", return_value=True
        ):
            # Execute
    ...
    def test_notification_create_disabled(self):
        ...
        # Setup
        with patch(
            "apps.notifications.services.should_send_notification", return_value=False
        ):
            # Execute
            notification = notification_create(
                recipient=self.user,
    ...
    def test_notification_create_batch_respects_preferences(self):
        ...
        with patch(
            "apps.notifications.services.should_send_notification",
            ...
            # Assert
            self.assertEqual(
                self.notification_create_mock.call_count, 1
            )  # Only in_app notification created
