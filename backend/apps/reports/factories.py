...
class ReportFactory(factory.django.DjangoModelFactory):
    ...
    description = factory.Faker("text")
    status = factory.LazyAttribute(lambda _: random.choice([status[0] for status in ReportStatus.choices]))
    evidence_links = factory.LazyAttribute(lambda _: {"link1": fake.url(), "link2": fake.url()})
