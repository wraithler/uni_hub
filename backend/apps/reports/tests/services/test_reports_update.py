...
class ReportUpdateTests(TestCase):
    ...
    def test_report_update_success(self):
        report = ReportFactory.create(title="Initial Report", description="Initial Description")
        updated_report = report_update(
            report=report, 
            data={"title": "Updated Report", "description": "Updated Description", "evidence_links": {"link1": "https://example.com"}}
        )
